...
impl SessionManager {
    ...
    pub async fn update_session(&self, session: Session) -> MCPResult<()> {
        ...
        if !sessions.contains_key(&session.id) {
            return Err(MCPError::Session(SessionError::NotFound(
                session.id.clone(),
            )));
        }
    ...
    pub async fn update_activity(&self, session_id: &str) -> MCPResult<()> {
        ...
        if let Some(session) = sessions.get_mut(session_id) {
            ...
            Err(MCPError::Session(SessionError::NotFound(
    ...
    pub async fn create_execution_context(
        ...
        tool_registry: Option<Arc<crate::mcp::tools::ToolRegistry>>,
