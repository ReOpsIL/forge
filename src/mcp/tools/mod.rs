...
pub struct ExecutionContext {
    ...
    /// Tool registry for delegating to other MCP tools
    pub tool_registry: Option<std::sync::Arc<ToolRegistry>>,

    /// Claude session manager for sending prompts to running Claude CLI
    pub claude_session_manager: Option<std::sync::Arc<crate::models::ClaudeSessionManager>>,
}
...
pub enum Content {
    ...
    #[serde(rename = "binary")]
    Binary { data: String, content_type: String },
...
impl PerformanceTracker {
    ...
    pub fn get_tool_statistics(&self, tool_name: &str) -> ToolStatistics {
        let tool_executions: Vec<_> = self
            .executions
            .iter()
        ...
        let total_time: Duration = tool_executions.iter().filter_map(|e| e.duration).sum();
