...
use crate::mcp::tools::{
    Content, ExecutionContext, MCPTool, PerformanceTracker, Permission, ToolCategory, ToolError,
    ToolExecution, ToolResult, ToolStatistics,
};
...
impl ToolRegistry {
    ...
    pub async fn unregister_tool(&self, name: &str) -> MCPResult<()> {
        ...
        if tools.remove(name).is_none() {
            return Err(MCPError::ToolExecution(
                crate::mcp::errors::ToolError::NotFound(name.to_string()),
            ));
        }
    ...
    pub async fn execute_tool(
        ...
        // Check permissions
        self.check_permissions(tool.as_ref(), &context.permissions)
            .await?;
        ...
        // Execute the tool
        let result =
            match tokio::time::timeout(self.config.default_timeout, tool.execute(params, context))
                .await
            {
                Ok(Ok(mut result)) => {
    ...
    pub async fn execute_tools_parallel(
        ...
        for (tool_name, params) in tool_calls {
            ...
            let handle = tokio::spawn(async move {
                ...
                registry
                    .execute_tool(&tool_name, params, &mut context_clone)
                    .await
            });
    ...
    async fn validate_tool(&self, tool: &dyn MCPTool) -> MCPResult<()> {
        ...
        if !schema.is_object() {
            return Err(MCPError::Validation(
                "Tool input schema must be a JSON object".to_string(),
            ));
        }
    ...
    async fn update_statistics(&self, tool_name: &str, execution: &ToolExecution) {
        ...
        tool_stats.failure_rate = (tool_stats.total_executions - tool_stats.successful_executions)
            as f32
...
/// Manual Debug implementation for ToolRegistry since it contains trait objects
impl std::fmt::Debug for ToolRegistry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ToolRegistry")
            .field("config", &self.config)
            .field("tools", &"<HashMap<String, Box<dyn MCPTool>>>")
            .field("metadata", &"<Tool Metadata>")
            .field("statistics", &"<Tool Statistics>")
            .field("performance_tracker", &"<Performance Tracker>")
            .finish()
    }
}
