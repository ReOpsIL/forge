...
pub struct MCPServer {
    ...
    /// Claude session manager for prompt injection
    claude_session_manager: Option<Arc<crate::models::ClaudeSessionManager>>,
...
impl MCPServer {
    ...
    pub async fn new(
        ...
        info!(
            "MCP Server created with working directory: {}",
            server.config.working_directory.display()
        );
        Ok(server)
    }
    ...
    pub async fn handle_connection(
        ...
        info!(
            "New connection: {} ({})",
            connection_id,
            transport.transport_type() as u8
        );
        ...
        loop {
            match transport.receive().await {
                Ok(message) => {
                    if let Err(e) = self
                        .handle_message(message, &mut transport, &connection_id, &mut session_id)
                        .await
                    {
        ...
        // Cleanup connection
        self.cleanup_connection(&connection_id, session_id.as_deref())
            .await;
    ...
    async fn handle_request(
        ...
        let response = match request.method.as_str() {
            ...
            _ => Err(MCPError::Server(ServerError::MethodNotFound(
    ...
    async fn handle_initialize(&self, params: Option<Value>) -> MCPResult<Value> {
        ...
        let result = InitializeResult {
            ...
            capabilities: ServerCapabilities {
                ...
                resources: crate::mcp::protocol::ResourcesCapability {
                    list_changed: true,
                    subscribe: false,
                },
    ...
    async fn handle_tool_call(
        ...
        let session_id_str = if let Some(id) = session_id {
            ...
            info!(
                "No session found for tool '{}', creating a temporary session",
                tool_name
            );
            ...
            let new_session_id = self
                .session_manager
                .create_temp_session(client_info)
                .await?;
        ...
        let mut context = self
            ...
            .create_execution_context(
                ...
                self.claude_session_manager.clone(),
            )
            .await?;
        ...
        let result = self
            .tool_registry
            .execute_tool(tool_name, tool_params, &mut context)
            .await
        ...
        if created_temp_session {
            if let Err(e) = self
                .session_manager
                .terminate_session(&session_id_str)
                .await
            {
                warn!(
                    "Failed to terminate temporary session {}: {}",
                    session_id_str, e
                );
            } else {
    ...
    async fn register_builtin_tools(registry: &Arc<ToolRegistry>) -> MCPResult<()> {
        ...
        registry
            .register_tool(Box::new(CreateDirectoryTool))
            .await?;
        ...
        registry.register_tool(Box::new(UpdateBlockTool)).await?;
