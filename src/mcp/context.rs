...
impl ContextStore {
    ...
    pub fn set_shared(&mut self, key: impl Into<String>, value: Value, tool_name: impl Into<String>) -> MCPResult<()> {
        let key = key.into();
        let tool_name = tool_name.into();
        
        // Check limits
        if self.shared_data.len() >= self.config.max_shared_entries && !self.shared_data.contains_key(&key) {
            return Err(MCPError::Context(ContextError::StorageLimit("Shared data limit exceeded".to_string())));
        }
    ...
    pub fn get_shared(&mut self, key: &str) -> Option<Value> {
        ...
        if let Some(entry) = self.shared_data.get_mut(key) {
            ...
            if self.cache.len() < self.config.max_cache_size {
                self.cache.insert(key.to_string(), CachedEntry {
                    value: value.clone(),
    ...
    pub fn set_session(&mut self, session_id: impl Into<String>, key: impl Into<String>, value: Value, tool_name: impl Into<String>) -> MCPResult<()> {
        let session_id = session_id.into();
        let key = key.into();
        let tool_name = tool_name.into();
        
        let session_data = self.session_data.entry(session_id.clone()).or_insert_with(HashMap::new);
        
        // Check limits
        if session_data.len() >= self.config.max_session_entries && !session_data.contains_key(&key) {
            return Err(MCPError::Context(ContextError::StorageLimit("Session data limit exceeded".to_string())));
        }
        ...
        debug!("Set session context value: {} in session {}", key, session_id);
        Ok(())
    }
    ...
    pub fn delete_session(&mut self, session_id: &str, key: &str, tool_name: impl Into<String>) -> MCPResult<bool> {
        let tool_name = tool_name.into();
        ...
        if let Some(session_data) = self.session_data.get_mut(session_id) {
            if let Some(entry) = session_data.remove(key) {
                ...
                debug!("Deleted session context value: {} in session {}", key, session_id);
                Ok(true)
            } else {
                Ok(false)
            }
    ...
    pub fn set_read_only(&mut self, key: &str, session_id: Option<&str>, read_only: bool) -> MCPResult<()> {
        if let Some(session_id) = session_id {
            if let Some(session_data) = self.session_data.get_mut(session_id) {
                if let Some(entry) = session_data.get_mut(key) {
    ...
    pub fn add_tags(&mut self, key: &str, session_id: Option<&str>, tags: Vec<String>) -> MCPResult<()> {
        if let Some(session_id) = session_id {
            if let Some(session_data) = self.session_data.get_mut(session_id) {
                if let Some(entry) = session_data.get_mut(key) {
    ...
    pub fn search_by_tags(&self, tags: &[String], session_id: Option<&str>) -> Vec<String> {
        ...
        let data = if let Some(session_id) = session_id {
            self.session_data.get(session_id).map(|d| d as &HashMap<String, ContextEntry>)
        } else {
    ...
    pub fn cleanup_expired(&mut self) -> usize {
        ...
        // Clean shared data
        let expired_shared: Vec<String> = self.shared_data
            .iter()
            .filter(|(_, entry)| {
        ...
        // Clean cache
        let expired_cache: Vec<String> = self.cache
            .iter()
            .filter(|(_, entry)| {
    ...
    pub fn get_statistics(&self) -> ContextStatistics {
        ...
        let most_accessed_shared = self.shared_data
            .iter()
...
mod tests {
    ...
    fn test_shared_context() {
        ...
        // Set value
        assert!(store.set_shared("test_key", json!("test_value"), "test_tool").is_ok());
    ...
    fn test_session_context() {
        ...
        // Set value
        assert!(store.set_session("session1", "test_key", json!("test_value"), "test_tool").is_ok());
        
        // Get value
        assert_eq!(store.get_session("session1", "test_key"), Some(json!("test_value")));
    ...
    fn test_read_only() {
        ...
        // Set value
        store.set_shared("readonly_key", json!("value"), "test_tool").unwrap();
