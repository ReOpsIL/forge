...
impl UnifiedStateManager {
            ...
            if let Some(project) = state.projects.get_mut(project_id) {
                for (field, new_value) in updates {
                    let old_value = match field.as_str() {
                        ...
                        "status" => {
                            if let Ok(status) = serde_json::from_value::<ProjectStatus>(new_value.clone()) {
        ...
        self.update_core_state(|state| {
            if let Some(task) = state.tasks.get_mut(task_id) {
                ...
                if let Some(progress) = progress {
                    ...
                    changes.insert("progress".to_string(), StateChange {
                    ...
                    TaskStatus::Completed | TaskStatus::Failed | TaskStatus::Cancelled => {
                        ...
                        if let Some(started_at) = task.started_at {
                            if let Ok(duration) = SystemTime::now().duration_since(started_at) {
                                let total_duration = state.performance_metrics.average_task_duration * (state.performance_metrics.total_tasks_executed - 1) as u32 + duration;
                                state.performance_metrics.average_task_duration = total_duration / state.performance_metrics.total_tasks_executed as u32;
                            }
                        }
                    }
                    _ => {}
        ...
        self.update_core_state(|state| {
            if let Some(block) = state.blocks.get_mut(block_id) {
                ...
                if let Some(outputs) = outputs {
                    ...
                    changes.insert("outputs".to_string(), StateChange {
    ...
    pub fn get_statistics(&self) -> StateStatistics {
        ...
        StateStatistics {
            ...
            active_tasks: core_state.tasks.values().filter(|t| matches!(t.status, TaskStatus::Running | TaskStatus::Pending)).count(),
            total_blocks: core_state.blocks.len(),
            active_sessions: core_state.sessions.values().filter(|s| s.status == SessionStatus::Active).count(),
