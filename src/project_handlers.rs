                ...
                if !project_dir.exists() {
                    match std::fs::create_dir_all(project_dir) {
                        Ok(_) => println!(
                            ...
                            config.project_home_directory
                        ),
                        Err(e) => {
                            eprintln!("Error creating project directory: {}", e);
                            return HttpResponse::InternalServerError()
                                .body(format!("Error creating project directory: {}", e));
                        }
                    }
                }
            }
...
pub async fn get_professions_handler() -> impl Responder {
    ...
    let mut categories_map: std::collections::HashMap<ProfessionCategory, Vec<ProfessionResponse>> =
...
pub async fn get_profession_prompts_handler(path: web::Path<String>) -> impl Responder {
    ...
    if let Some(profession) = profession_prompts::get_profession_by_id(&profession_id) {
        ...
        let prompts_response = ProfessionPromptsResponse {
            ...
            process_specification_system_prompt: profession
                .prompts
            ...
            process_specification_system_prompt_mcp: profession
                .prompts
                .process_specification_system_prompt_mcp,
            process_specification_user_prompt_mcp: profession
                .prompts
