...
pub async fn get_blocks_from_file_handler(data: web::Data<AppState>) -> impl Responder {
    match data.block_manager.load_blocks_from_file() {
...
async fn enhance_block_with_llm(
    mut block: Block,
    data: &web::Data<AppState>,
) -> Result<Block, String> {
...
pub async fn enhance_block_handler(
    block: web::Json<Block>,
    data: web::Data<AppState>,
) -> impl Responder {
...
pub async fn generate_tasks_block_handler(
    block: web::Json<Block>,
    data: web::Data<AppState>,
) -> impl Responder {
    let mut block = block.into_inner();
...
async fn generate_tasks_with_llm(
    mut block: Block,
    data: &web::Data<AppState>,
) -> Result<Block, String> {
    // Get the project configuration to get the LLM provider setting
    ...
    let generated_tasks = generate_tasks(&block, &data.claude_session_manager).await?;
...
pub async fn get_block_dependencies_handler(
    ...
    let block = match blocks.iter().find(|b| b.block_id == block_id) {
        Some(block) => block,
        None => {
            return HttpResponse::NotFound().body(format!("Block with ID {} not found", block_id));
        }
    };

    // Extract tasks and their dependencies from the block's todo_list
    let tasks: Vec<TaskDependency> = block
        .todo_list
        .values()
...
pub async fn add_task_handler(
    path: web::Path<String>,
    task_request: web::Json<TaskItemRequest>,
    data: web::Data<AppState>,
) -> impl Responder {
    let block_id = path.into_inner();
    ...
    match data
        .block_manager
        .update_block(blocks[block_index.unwrap()].clone())
    {
        Ok(_) => {
            // Save the updated blocks to the file
            if let Err(e) = data.block_manager.save_blocks_to_file() {
...
pub async fn remove_task_handler(
...
pub async fn generate_sample_config_handler() -> impl Responder {
    match generate_sample_config(BLOCK_CONFIG_FILE) {
        ...
        Err(e) => HttpResponse::InternalServerError()
            .body(format!("Failed to generate sample config: {}", e)),
    }
}
...
pub async fn process_markdown_handler(
    request: web::Json<ProcessMarkdownRequest>,
    data: web::Data<AppState>,
) -> impl Responder {
...
pub async fn process_specification_handler(
    request: web::Json<ProcessSpecRequest>,
    data: web::Data<AppState>,
) -> impl Responder {
    let request = request.into_inner();

    // Get the project configuration to get the LLM provider setting
    let _project_config = match data.project_manager.get_config() {
        Ok(config) => config,
        Err(e) => {
    ...
    match process_specification(&request.markdown_content, &data.claude_session_manager).await {
        Ok(message) => {
...
pub fn create_blocks_from_llm(
    generated_blocks: Vec<GeneratedBlock>,
    data: web::Data<AppState>,
) -> Result<(ProcessSpecResponse), String> {
    ...
    let response = ProcessSpecResponse {
        status: "success".to_string(),
        message: format!(
            "Successfully processed specification and created {} blocks",
            created_blocks.len()
        ),
